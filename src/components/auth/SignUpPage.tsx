...
import { User, Mail, Lock, ShieldCheck, AlertCircle, ArrowLeft, Eye, EyeOff } from 'lucide-react';
...
export const SignUpPage: React.FC<SignUpPageProps> = ({ onNavigateToSignIn, onNavigateToHome }) => {
  ...
  // Synchronous submission guard to prevent duplicate concurrent sign-up requests
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Sign up error:", err?.code || err?.message || err);
      ...
      <div className="max-w-md w-full mx-auto bg-clay rounded-3xl p-8 border border-warm card-shadow space-y-6">
        ...
        <div className="space-y-2 text-center">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-[#fdfcf8] text-olive border border-warm">
            <ShieldCheck className="w-3.5 h-3.5 text-olive" /> Nutri Guide Account
          </div>
          <h2 className="serif text-2xl font-bold text-[#2c3333] tracking-tight">
            Create Your Account
          </h2>
          <p className="text-xs text-slate-600 leading-relaxed">
            Register to access your personal nutrition dashboard and saved preferences.
          </p>
        </div>
